...
class ROIAnalyzerApp:
    def __init__(self, root):
        ...
        self.icons_path = {
            ...
            'batch' : current_dir + "\\images\\buttons\\batch_32.png"
        }
        ...
        ####################### addition alice 
        # slice start/end vides par défaut, avec bouton “...” pour capturer l’indice courant
        self.create_label_and_entry(
            self.canvas1,
            "slice start:",
            default_value='',
            attr_name="slice_start"
        )
        self.create_label_and_entry(
            self.canvas1,
            "slice end:",
...
###################################
    ...
    def filter_action(self):
        ...
        ###########################################################
 # Dans syn_catch_GUI.py, à l’intérieur de ROIAnalyzerApp, remplacez
# votre méthode batch_binarize_action par ce code complet :

    def batch_binarize_action(self):
        # Empêche de relancer deux fois en même temps
        ...
        try:
            ...
            for s in range(big_start, big_end + 1, window_length):
                ...
                # Met à jour vos entrées pour refléter la tranche
    ...
    def binarize_action(self, return_df=False):
        ...
        """
        df, rows = self.prepare_data()
        results = []
        total = len(rows)
        filter_radius = int(self.filter_radius_entry.get())
        for idx, row_idx in enumerate(rows):
            fp = df.iloc[row_idx]['filepath']
            slice_start = self.parse_entry(self.slice_start_entry.get())
            slice_end   = self.parse_entry(self.slice_end_entry.get())
            tch = int(self.target_ch_entry.get()) - 1
            dch = int(self.second_ch_entry.get())   - 1

            stack_image(fp, slice_start, slice_end, tch, dch)
            location = df.iloc[row_idx]['location']
            files_out = filter_after_roi_selection(filter_radius, fp, location)
            self.add_file_links_from_list([files_out])
            
            if return_df:
                # on récupère le DataFrame renvoyé par binarize_images
...
####################################
    ...
    def define_hist_action(self):
        ...
        for s in range(big_start, big_end+1, window_length):
            ...
            # … ici tu déplaces ton for idx, row_idx in rows_to_process …
